...
export default function Sidebar() {
  ...
  return (
    <div
      className={` sm:hiden right-0 top-0 fixed duration-300 z-30 transition-all ease-out overflow-hidden ${
        sidebar ? "w-60" : "w-0"
      } h-screen bg-dark-100`}
    >
...
function SidebarLink({
  ...
  return (
    <Link onClick={onClick} href={link}>
      <h1 className="text-light-100 my-5 font-chivo font-bold text-xl">
        {title}
      </h1>
    </Link>
  );
